	...
	public function doWebManage() {
		...
		if (!$op){
			$list = pdo_fetchall('select m.*,r.name from '.tablename($this->modulename.'_rule')." m left join "
					.tablename('rule')." r on r.id=m.rid "." where m.weid='{$_W['uniacid']}' order by rid desc");
// 			//预约列表
			foreach ($list as $key => $value) {
				$list[$key]['book'] = pdo_fetchcolumn('select count(id) from '.tablename($this->modulename."_record")." where rid='{$value['rid']}' and weid='{$_W['uniacid']}'");
			}
	...
	public function doWebEditCate(){
		...
		if(!empty($cates) && $type != 0){
			if($type == 1){
				if(pdo_update($this->modulename.'_cate',array('title'=>$new_cate),array('id'=>$cates['id'],'weid'=>$weid)) == false)
					echo 0;
				else{
					echo 1;
				}
			...
			if($type == 0){
				$cates = pdo_fetch('select * from '.tablename($this->modulename.'_cate').' where title=:title',array('title'=>$cate,'weid'=>$weid));
				if(!empty($cates)){
					die('-1');
				}
	...
	public function doMobileIndex(){
		global $_W,$_GPC;
		$rid = $_GPC['rid'];
		$rule = pdo_fetch('select * from '.tablename($this->modulename."_rule")." where rid='{$rid}' and weid='{$_W['uniacid']}'");
		$list = pdo_fetchall('select * from '.tablename($this->modulename."_list")." where cate='{$rule['cate']}' and weid='{$_W['uniacid']}' order by displayorder desc");
	...
	public function doMobileRecord(){
		...
		$from_user = $_W['openid'];
		$record = pdo_fetch('select * from '.tablename($this->modulename."_record")." where rid='{$rid}' and lid='{$lid}' and mobile='{$mobile}' and weid='{$_W['uniacid']}' and status=0 limit 1");
		$title = pdo_fetchcolumn('select title from ' . tablename($this->modulename."_list") . "where id={$lid} and weid='{$_W['uniacid']}'");
// 		$type = pdo_fetchcolumn('select c.title from ' . tablename($this->modulename."_list") . " l left join " . tablename($this->modulename."_cate") . " c on c.id=l.cate where l.id={$lid} and l.weid='{$_W['uniacid']}'");
		$cfg = $this->module['config'];
		$from_admin = explode(',', $cfg['MSGID']['adminmsg']);
		if (!empty($record)){
			message('该手机号码已经预约了哦！');
		}
		if (pdo_insert($this->modulename."_record",array('weid'=>$_W['uniacid'],'rid'=>$rid,'lid'=>$lid,'realname'=>$realname,'mobile'=>$mobile,'visitetime'=>strtotime($visitetime),'createtime'=>time(),'openid'=>$from_user,'num'=>$num)) === false){
			message('预约失败，请联系管理员！');
		}else {
			$this->sendBookingMsg($from_user,$title,$visitetime,$num);
			foreach ($from_admin as $f) if (!empty($f)) $this->sendAdminMsg($f, $title, $realname, $mobile, $visitetime,$num);
			message('预约成功！',$this->createMobileUrl('index',array('rid'=>$rid)));
		}
	}
	
	function sendBookingMsg($from_user,$title,$time,$num){
		global $_W;
		$cfg = $this->module['config'];
		$msgid = $cfg['MSGID']['booking_msg'];
		$url = $cfg['MSGID']['booking_msg_link'];
		if (!empty($msgid)){
			$data = array(
					'first' => array(
							'value' => "您好，您已{$title}成功",
					),
					'name' => array(
							'value' => $title,
					),
					...
					'remark' => array(
							'value' => !empty($cfg['MSGID']['mobile']) ? "如有疑问，请咨询{$cfg['MSGID']['mobile']}" : "期待您的光临！",
					)
			);
			$this->sendTemplateMsg($from_user,$msgid,$data,$url);
		}
	}
	
	function sendAdminMsg($from_user,$title,$realname,$mobile,$time,$num){
		global $_W;
		$cfg = $this->module['config'];
		$msgid = $cfg['MSGID']['reming_msg'];
		$url = $cfg['MSGID']['reming_msg_link'];
		if (!empty($msgid)){
			$data = array(
					'first' => array(
							'value' => "客户预约提醒!\n预约人数：".$num."人",
...
// 		}
	...
	public function doMobileImg(){
		global $_W,$_GPC;
		$lid = $_GPC['lid'];
		$list = pdo_fetch('select * from '.tablename($this->modulename."_list")." where id='{$lid}' and weid='{$_W['uniacid']}'");
		die("
